...
class StockWarehouseOrderpointWizard(models.TransientModel):
    ...
    to_recalculate = fields.Boolean("¿Desea recalcular la cantidad a pedir en las categorías seleccionadas?")
    ...
    def action_confirm(self):
        ...
        ctx.update(
            {
                ...
                "to_recalculate": self.to_recalculate,
            }
        )
        ...
        # Si "to_recalculate" está activado...
        if ctx.get("to_recalculate"):
            # Verifico si hay al menos un filtro aplicado
            has_filters = any(
                [ctx["filter_products"], ctx["filter_categories"], ctx["filter_locations"], ctx["filter_suppliers"]]
            )

            if has_filters:
                # 🔹 Si hay filtros, aplico el filtrado
                filtered_orderpoints = orderpoints.filtered(
                    lambda op: op.product_id.id in ctx["filter_products"]
                    or op.product_category_id.id in ctx["filter_categories"]
                    or op.location_id.id in ctx["filter_locations"]
                    or op.supplier_id.partner_id.id in ctx["filter_suppliers"]
                    or op.product_id.seller_ids.partner_id.id in ctx["filter_suppliers"]
                )
            else:
                # 🔹 Si no hay filtros, se recalculan TODOS los orderpoints
                filtered_orderpoints = orderpoints

            filtered_orderpoints._change_review_toggle_negative()
